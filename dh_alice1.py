...
def run(addr, port):
    ...
    if not is_valid_prime(p):
        logging.error("Received invalid prime number")
        conn.close()
        return

    if not is_valid_generator(g, p):
        logging.error("Received invalid generator")
        conn.close()
        return
    ...
    bob_encrypted_msg = base64.b64decode(recv_all(conn)["encryption"])
    decrypted_msg_from_bob = AES.new(aes_key, AES.MODE_ECB).decrypt(bob_encrypted_msg)
    ...
    # 메시지 암호화
    encrypted_msg_to_bob = AES.new(aes_key, AES.MODE_ECB).encrypt(padded_msg.encode())

    # Base64 인코딩
    encrypted_msg_to_bob_b64 = base64.b64encode(encrypted_msg_to_bob).decode("utf-8")
